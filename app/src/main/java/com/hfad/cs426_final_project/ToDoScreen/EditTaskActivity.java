...
public class EditTaskActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getExtras() != null) {
            userTask = (UserTask) getIntent().getExtras().get("current_task");
        }
    ...
    private void initCurrentTask() {
        ...
        edtTaskStartDate.setText(CalendarUtils.convertMillisToDate(userTask.getStartDate()));
        ...
        for(int i = 0; i < tagList.size(); i++) {
            if(tagList.get(i).sameID(userTask.getTag())) {
                curTagPos = i;
                break;
            }
        }
        searchTagSpinner.setSelection(curTagPos);
    }
}